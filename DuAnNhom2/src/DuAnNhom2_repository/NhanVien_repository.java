 ...
 */
package DuAnNhom2_repository;

import DuAnNhom2_model.NhanVien;
import DuAnNhom2_utility.JDBC_Helper;
...
public class NhanVien_repository {
    ...
    public static int sua(NhanVien nv) {
        String sql = "update NhanVien set HOTEN = ?, GIOITINH = ?, NgaySinh = ?, DIACHI = ?, SDT = ?,"
                + " EMAIL = ?, TENTAIKHOAN = ?, MATKHAU = ?, TrangThai = ?, HINHANH = ? where MANV=? ";
       return JDBC_Helper.updateTongQuat(sql,  nv.getHoTen(), nv.getGioiTinh(), nv.getNgaySinh(), nv.getDiaChi(),nv.getSoDienThoai(), nv.getEmail(),  nv.getTenTaiKhoan(), nv.getMatKhau(), nv.getTrangThai(), nv.getHinhAnh(), nv.getMaNhanVien());
    }
